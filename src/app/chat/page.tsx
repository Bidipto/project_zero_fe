  ...
  interface Message {
    ...
    id: string;
    timestamp: Date;
    status?: 'sending' | 'sent' | 'delivered' | 'read' | 'failed'; // added a failed status
  }
  ...
  const ChatPage = () => {
    ...
    const fetchUsers = useCallback(async (query: string) => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/users/search?q=${encodeURIComponent(query)}`, {
          headers: {
    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        try {
          ...
          const getUserData = await getPrivateChat();
          // Optionally set recentChats or activeChat from getUserData if needed
          if (getUserData && Array.isArray(getUserData)) {
            setRecentChats(getUserData.map((chat: any) => ({
              id: chat.id,
              name: chat.title || chat.participants?.[0]?.username || `Chat ${chat.id}`,
              status: 'online',
              avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(chat.title || chat.participants?.[0]?.username || `Chat ${chat.id}`)}&background=8b5cf6&color=fff&size=128`,
              lastSeen: new Date()
            })));
          }
          if (activeChat && activeChat.id) {
            const getChatMessagesData = await getChatMessages(activeChat.id);
            if (Array.isArray(getChatMessagesData)) {
              setMessages(getChatMessagesData.map((msg: any) => ({
                id: msg.id,
                text: msg.content,
                sender: msg.sender,
            ...
            if (inputMessage && inputMessage.trim()) {
              await sendMessagesToChat(inputMessage);
            }
    ...
    const handleStartNewChat = useCallback(async (selectedUser: User) => {
      try {
        const response = await fetch(`${EnvironmentVariables.BACKEND_URL}/v1/chats`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
        ...
        setUserListError('Failed to start chat. Please try again.');
      }
    }, []);
    ...
    }, [newChatUsername, handleCreateNewChat]);
    ...
    const getChatMessages = async (chatId: string) => {
      try {
        if (!activeChat || !activeChat.id) throw new Error('No active chat selected');
        const token = localStorage?.getItem('access_token');
        const headers: Record<string, string> = {
          'Content-Type': 'application/json',
          'accept': 'application/json'
        ...
        const res = await fetch(`${EnvironmentVariables.BACKEND_URL}/v1/chat/${activeChat.id}/messages`, {
          method: 'GET',
          headers: headers
        });
        console.log(res.status);
    ...
    const sendMessagesToChat = async (message: string) => {
      if (!activeChat || !activeChat.id) throw new Error('No active chat selected');
      const token = localStorage?.getItem('access_token');
      const headers: Record<string, string> = {
        'Content-Type': 'application/json',
      ...
      // Use correct chat id and message format
      const chatId = activeChat.id;
      const res = await fetch(`${EnvironmentVariables.BACKEND_URL}/v1/chat/${chatId}/messages`, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
          content: message,
          message_type: 'text',
          chat_id: chatId // ensure chat_id is sent if required by backend
        })
      });
      if (!res.ok) {
        const errorData = await res.text();
    ...
    const markMessagesAsRead = async (chatId: string) => {
      ...
      const res = await fetch(`${EnvironmentVariables.BACKEND_URL}/v1/chat/${chatId}/messages/mark-read`, {
        method: 'POST',
        headers: headers
      });
      console.log(res.status);
    ...
    const createPrivateChat = async (username: string) => {
      ...
      // Add to recent chats
      setRecentChats((prev) => 
        prev.some((chat) => chat.id === newUser.id) ? prev : [newUser, ...prev]
      )
        try{
        const token = localStorage.getItem('access_token')
        const headers: Record<string, string> = {
        ...
        <AnimatePresence>
          {showStartChatModal && (
            ...
            >
              ...
              {username.length === 0 ? (
                <li className="text-center text-gray-400 py-8">No users found.</li>
              ) : (
                username.map((user) => (
                  <motion.li
                    key={user.id}
