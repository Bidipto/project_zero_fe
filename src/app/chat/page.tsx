...
const ChatPage = () => {
  ...
  const validateAndConvertId = useCallback((id: string | number): number | string => {
  ...
  const fetchUsers = useCallback(async (query: string) => {
    try {
      ...
      const response = await fetch(`${EnvironmentVariables.BACKEND_URL}/v1/user/usernames?search=${encodeURIComponent(query)}`, {
        method: 'GET',
        headers
      });
      ...
      const usernames = data.usernames || data.data || data || [];
      const transformedUsers: User[] = usernames
        .filter((user: any) => typeof user === 'string' ? user.toLowerCase().includes(query.toLowerCase()) : 
      ...
      setUserListError('Failed to fetch users. Please try again.');
      setUsers([]);
    }
  }, []);
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      if (searchQuery.trim()) {
        ...
        setUsers(username); // Reset to all users when search is cleared
      }
    }, 300); 
    return () => {
      clearTimeout(handler);
    };
  }, [searchQuery, fetchUsers, username]); 
  ...
  const getChatMessages = useCallback(async (chatId: string | number) => {
    try {
      ...
      const numericChatId = validateAndConvertId(chatId);
      
      const res = await fetch(`${EnvironmentVariables.BACKEND_URL}/v1/chat/${numericChatId}/messages`, {
        method: 'GET',
        headers: headers
      });
  ...
  const sendMessagesToChat = useCallback(async (message: string, chatId: string | number) => {
    if (!chatId) throw new Error('No chat ID provided');
    const token = localStorage?.getItem('access_token');
    const headers: Record<string, string> = {
      'Content-Type': 'application/json',
  ...
  const markMessagesAsRead = useCallback(async (chatId: string | number) => {
    const token = localStorage?.getItem('access_token');
    const headers: Record<string, string> = {
      'Content-Type': 'application/json',
      'accept': 'application/json'
    ...
    const res = await fetch(`${EnvironmentVariables.BACKEND_URL}/v1/chat/${numericChatId}/messages/mark-read`, {
      method: 'POST',
      headers: headers
    });
    ...
    if (!res.ok) {
      const errorData = await res.text();
      console.error('API Error Response:', errorData);
      throw new Error(`Failed to mark messages as read: ${res.status} - ${errorData}`);
    }
    const data = await res.json();
    return data;
  }, [validateAndConvertId]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 2. Get private chat data
        const chatData = await getPrivateChat();
        if (chatData && Array.isArray(chatData)) {
          setRecentChats(chatData.map((chat: any) => ({
            id: chat.id,
            name: chat.title || chat.participants?.[0]?.username || chat.other_user?.username || `Chat ${chat.id}`,
            status: 'online' as const,
            avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(chat.title || chat.participants?.[0]?.username || chat.other_user?.username || `Chat ${chat.id}`)}&background=8b5cf6&color=fff&size=128`,
            lastSeen: new Date()
          })));
        } else if (chatData && chatData.chats) {
          setRecentChats(chatData.chats.map((chat: any) => ({
            id: chat.id,
            name: chat.title || chat.participants?.[0]?.username || chat.other_user?.username || `Chat ${chat.id}`,
            status: 'online' as const,
            avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(chat.title || chat.participants?.[0]?.username || chat.other_user?.username || `Chat ${chat.id}`)}&background=8b5cf6&color=fff&size=128`,
        ...
        setUserListError('Failed to load data.');
        setUsers([]);
        setUsername([]);
      }
  ...
  useEffect(() => {
    ...
    const getMessages = async () => {
      try {
        ...
        const normalized = raw.map((m: any): Message => ({
          text: m.content ?? m.text ?? "",
          sender: m.sender === "user" || m.sender === "bot" ? m.sender :
                  (m.sender_id && m.sender_id === currentUser?.name) ? "user" : "bot",
          id: String(m.id ?? m._id ?? `msg-${Date.now()}-${Math.random().toString(36).slice(2)}`),
          timestamp: m.timestamp ? new Date(m.timestamp) : new Date(),
          status: m.is_read ? "read" : (m.status ?? "delivered"),
        }));
    ...
    getMessages();
  }, [activeChat, currentUser, getChatMessages, markMessagesAsRead]);

  // Cleanup function for the readTimeoutRef
  useEffect(() => {
    return () => {
  ...
  const handleStartNewChat = useCallback(async (selectedUser: User) => {
    try {
      // Create private chat with the selected user
      const chatData = await createPrivateChat(selectedUser.name);
      
      // Create user with the chat ID from the API response
      const newUser: User = {
        id: chatData.id || chatData.chat_id || selectedUser.id,
      ...
      setRecentChats((prev) => {
        const exists = prev.find((chat) => chat.name === newUser.name);
        return exists ? prev : [newUser, ...prev];
      });
      
    } catch (error) {
      console.error('Error starting new chat:', error);
      // Fallback: still set the chat but without API integration
      setActiveChat(selectedUser);
      setShowStartChatModal(false);
      setSearchQuery('');
      
      setRecentChats((prev) => {
        const exists = prev.find((chat) => chat.name === selectedUser.name);
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      // Send message to API
      await sendMessagesToChat(messageText, activeChat.id);
  ...
  }, [inputMessage, activeChat, sendMessagesToChat]);
  ...
  const handleCreateNewChatFromModal = useCallback(async () => {
    ...
    try {
      // Check if user exists in the username list
      const existingUser = username.find(user => 
        user.name.toLowerCase() === newChatUsername.toLowerCase()
      );
      ...
      if (existingUser) {
        ...
        const newUser: User = {
          id: chatData.id || chatData.chat_id || `new-${Date.now()}`,
      ...
      setUserListError('Failed to create chat. Please try again.');
    }
  }, [newChatUsername, username, handleStartNewChat, createPrivateChat]);
        ...
        {showStartChatModal && (
          ...
          >
            ...
            >
              ...
              <motion.div className="mb-8">
                ...
                <div className="relative">
                  <motion.input
                    ...
                    className="w-full py-4 px-6 bg-gray-900/70 rounded-2xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:bg-gray-800/80 border-2 border-transparent focus:border-purple-500 text-lg shadow-md transition-all duration-200"
                  ...
                  {users.length === 0 ? (
                    ...
                    users.map((user) => (
                      <motion.li
                        ...
                        initial="hidden"
                        animate="visible"
                        whileHover={{ scale: 1.03, backgroundColor: "#2a2040", boxShadow: "0 4px 24px #a78bfa22" }}
                        whileTap={{ scale: 0.98 }}
                        className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer border-2 transition-all duration-200 ${
                          activeChat?.id === user.id ? 'bg-purple-900/40 border-purple-500/60' : 'bg-gray-900/60 border-transparent hover:border-purple-400/40'
                        }`}
                      ...
                      >
                        <div className="relative flex-shrink-0">
                          ...
                          />
                          <span className={`absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-gray-900 ${statusColors[user.status]}`}></span>
                        </div>
                        <div className="flex-1 min-w-0">
              ...
              {newChatUsername.trim() && (
                ...
                >
                  <motion.button
                    ...
                    className="w-full py-4 px-6 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 rounded-2xl text-white font-semibold flex items-center justify-center gap-3 shadow-xl border border-purple-500/30 transition-all duration-200"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <motion.div className="mt-4 text-sm text-gray-400 text-center">
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-purple-900/20">
        {!activeChat ? (
          ...
          >
            ...
            >
              {/* <motion.div className="mb-8 p-8 bg-gradient-to-br from-purple-900/30 to-gray-800/30 rounded-3xl border border-purple-500/20 backdrop-blur-sm">
                <motion.div 
